...
public:
    ...
    BaseIterator<T>::const_reference operator[](int index) const;
    ...
    VectorConstIterator<T> &operator++() noexcept;   // Перегрузка инкремента
    ...
    VectorConstIterator<T> &operator--() noexcept; // Перегрузка декремента
